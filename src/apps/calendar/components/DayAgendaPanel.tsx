...
export const DayAgendaPanel: React.FC<DayAgendaPanelProps> = ({
  ...
  return (
    <div className="w-full md:w-80 border-t md:border-t-0 md:border-l border-white/10 bg-[#161b22] flex flex-col h-auto md:h-full">
      {/* Selected Day Header */}
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        ...
        {dayEvents.length === 0 ? (
          <div className="p-6 text-center rounded-2xl bg-white/[0.02] border border-dashed border-white/10 space-y-2">
            ...
            <p className="text-xs text-neutral-400">No events scheduled for this day</p>
            <button
              type="button"
              onClick={() => onOpenNewEvent(selectedDate)}
          ...
          dayEvents.map((event) => (
            <div
              ...
              className="p-3 rounded-2xl bg-white/[0.04] hover:bg-white/[0.08] border border-white/10 cursor-pointer transition-all space-y-2 group"
            >
              <div className="flex items-start justify-between">
                ...
                {event.syncedToGoogle && (
                  <span className="flex items-center gap-1 text-[9px] text-blue-400 font-medium bg-blue-500/10 px-1.5 py-0.5 rounded-md">
              ...
              <div className="flex flex-wrap items-center gap-3 text-[10px] text-neutral-400 pt-1 border-t border-white/5">
                ...
                {event.category && (
                  <span className="px-1.5 py-0.5 rounded-md bg-white/5 text-neutral-300">
                    {event.category}
                  </span>
                )}
              </div>
