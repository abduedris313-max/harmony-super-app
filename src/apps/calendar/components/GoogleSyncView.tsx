        ...
        {isGoogleConnected ? (
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={isSyncing}
              className="px-3 py-1.5 rounded-xl bg-blue-500/20 hover:bg-blue-500/30 text-blue-300 text-xs font-semibold flex items-center gap-1.5 transition-colors"
            >
            ...
            <button
              ...
              className="px-3 py-1.5 rounded-xl bg-white/5 hover:bg-white/10 text-neutral-400 hover:text-white text-xs font-semibold flex items-center gap-1.5 transition-colors"
            >
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className={`w-10 h-10 rounded-2xl flex items-center justify-center ${
              isGoogleConnected ? 'bg-blue-500/20 text-blue-400' : 'bg-white/10 text-neutral-500'
            }`}>
            ...
            <div>
              ...
              <p className="text-xs text-neutral-400">
                Authorized scope: <code className="font-mono text-blue-300">calendar.events</code>
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          {!isGoogleConnected ? (
            <div className="p-8 text-center rounded-2xl bg-white/[0.02] border border-white/5 space-y-2">
              <Cloud className="w-8 h-8 text-neutral-600 mx-auto" />
            ...
            <div className="p-8 text-center rounded-2xl bg-white/[0.02] border border-white/5 space-y-2">
              ...
              <p className="text-xs text-neutral-400">No upcoming events found on your Google Calendar</p>
            </div>
          ) : (
            <div className="space-y-2.5 max-h-[400px] overflow-y-auto pr-1">
              {googleEvents.map((gEv) => {
                ...
                return (
                  <div
                    key={gEv.id}
                    className="p-3.5 rounded-xl bg-white/[0.04] border border-white/10 space-y-2"
                  >
                    <div className="flex items-start justify-between gap-2">
                      <h4 className="text-xs font-bold text-white">{gEv.summary}</h4>
                      {isAlreadyImported ? (
                        <span className="px-1.5 py-0.5 rounded-md bg-emerald-500/20 text-emerald-400 text-[10px] font-medium shrink-0">
                          Imported
                        </span>
                      ) : (
                        <button
                          ...
                          className="px-2 py-1 rounded-md bg-blue-500/20 hover:bg-blue-500/30 text-blue-300 text-[10px] font-semibold shrink-0 transition-colors"
                        >
                          + Import
                        </button>
                      )}
        ...
        <div className="space-y-3">
          ...
          {unpushedLocalEvents.length === 0 ? (
            <div className="p-8 text-center rounded-2xl bg-white/[0.02] border border-white/5 space-y-2">
              ...
              <p className="text-xs text-neutral-400">All local events are synchronized with Google Calendar</p>
            </div>
          ) : (
            <div className="space-y-2.5 max-h-[400px] overflow-y-auto pr-1">
              {unpushedLocalEvents.map((lEv) => (
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    <h4 className="text-xs font-bold text-white">{lEv.title}</h4>
                    <button
                      type="button"
                      disabled={!isGoogleConnected}
                      onClick={() => handleExport(lEv)}
                      className="px-2 py-1 rounded-md bg-emerald-500/20 hover:bg-emerald-500/30 disabled:opacity-40 text-emerald-300 text-[10px] font-semibold shrink-0 transition-colors"
                    >
                      Push Event
                    </button>
                  </div>
