...
export const LoanTrackerView: React.FC<LoanTrackerViewProps> = ({
  ...
  return (
    <div className="space-y-4 text-white">
      ...
      <div className="p-4.5 rounded-2xl bg-gradient-to-br from-[#1c2230] via-[#161b22] to-[#0f131a] border border-[#30363d] shadow-xl">
        <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
          <div>
            ...
            <h2 className="text-2xl sm:text-3xl font-bold font-mono text-rose-400 mt-0.5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-3 border-t border-[#30363d]/70 text-xs">
          <div className="p-2.5 rounded-xl bg-[#0d1117] border border-[#30363d]/60">
            <span className="text-[10px] text-[#8b949e] uppercase font-bold">Monthly Combined EMI</span>
            <p className="text-base font-bold font-mono text-white mt-0.5">
              {formatCurrency(totalMonthlyEmi, currencyCode)} <span className="text-xs font-normal text-indigo-300">/mo</span>
            </p>
          </div>

          <div className="p-2.5 rounded-xl bg-[#0d1117] border border-[#30363d]/60">
            <span className="text-[10px] text-[#8b949e] uppercase font-bold">Total Paid Back So Far</span>
          ...
          <div className="p-2.5 rounded-xl bg-[#0d1117] border border-[#30363d]/60">
            <span className="text-[10px] text-[#8b949e] uppercase font-bold">Active Accounts</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="space-y-2.5 lg:col-span-1">
          ...
          {loans.length > 0 ? (
            loans.map((loan) => {
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between">
                    <div>
                      <div className="flex items-center gap-1.5">
                        <h4 className="text-xs font-bold text-white">{loan.title}</h4>
                  ...
                  <div className="flex items-center justify-between text-[10px] text-[#8b949e] mt-1.5">
            ...
            <div className="p-8 text-center text-[#8b949e] rounded-2xl bg-[#161b22] border border-[#30363d]">
              <Landmark className="w-8 h-8 mx-auto text-[#8b949e]/40 mb-2" />
        ...
        <div className="lg:col-span-2 space-y-3">
          {selectedLoan ? (
            <>
              ...
              <div className="p-4.5 rounded-2xl bg-[#161b22] border border-[#30363d] space-y-3 shadow-sm">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div>
                    <h3 className="text-base font-bold text-white">{selectedLoan.title}</h3>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs font-mono">
                  <div className="p-2.5 rounded-xl bg-[#0d1117] border border-[#30363d]/60">
                    <span className="text-[10px] text-[#8b949e] uppercase font-bold block font-sans">Remaining Principal</span>
                  ...
                  <div className="p-2.5 rounded-xl bg-[#0d1117] border border-[#30363d]/60">
                    <span className="text-[10px] text-[#8b949e] uppercase font-bold block font-sans">Monthly EMI</span>
                  ...
                  <div className="p-2.5 rounded-xl bg-[#0d1117] border border-[#30363d]/60">
                    <span className="text-[10px] text-[#8b949e] uppercase font-bold block font-sans">Annual Interest (APR)</span>
                    <span className="text-sm font-bold text-amber-400">{selectedLoan.interestRate}%</span>
                  </div>

                  <div className="p-2.5 rounded-xl bg-[#0d1117] border border-[#30363d]/60">
                    <span className="text-[10px] text-[#8b949e] uppercase font-bold block font-sans">Next Due Date</span>
                    <span className="text-sm font-bold text-indigo-300">{selectedLoan.nextDueDate}</span>
                  </div>
                </div>

                {selectedLoan.notes && (
                  <p className="text-xs text-[#8b949e] italic p-2 rounded-xl bg-[#0d1117] border border-[#30363d]/60">
                    "{selectedLoan.notes}"
                  </p>
                )}
              </div>
              ...
              <div className="p-4 rounded-2xl bg-[#161b22] border border-[#30363d] space-y-2.5">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-[#8b949e]">Monthly Annuity Breakdown</span>
                </div>
                  ...
                  <table className="w-full text-[11px] text-left border-collapse">
                    <thead>
                      <tr className="border-b border-[#30363d] text-[#8b949e] font-mono">
                        ...
                        <th className="py-1.5 px-2 text-emerald-400">Principal</th>
                        <th className="py-1.5 px-2 text-amber-400">Interest</th>
                        <th className="py-1.5 px-2">Remaining</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-[#30363d]/50 font-mono">
                      {amortization.map((item) => (
                        <tr key={item.month} className="hover:bg-[#21262d]/50">
                          <td className="py-1 px-2 text-[#8b949e]">#{item.month}</td>
                          <td className="py-1 px-2 text-white">{formatCurrency(item.payment, currencyCode)}</td>
                          <td className="py-1 px-2 text-emerald-400">+{formatCurrency(item.principal, currencyCode)}</td>
                          <td className="py-1 px-2 text-amber-400">-{formatCurrency(item.interest, currencyCode)}</td>
                          <td className="py-1 px-2 text-white font-bold">{formatCurrency(item.remainingBalance, currencyCode)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <div className="p-4 rounded-2xl bg-[#161b22] border border-[#30363d] space-y-2">
                  ...
                  <div className="space-y-1.5">
                    {selectedLoan.paymentHistory.map((rec) => (
                      <div
                        key={rec.id}
                        className="p-2 rounded-xl bg-[#0d1117] border border-[#30363d]/60 flex items-center justify-between text-xs"
                      >
                        <div>
                          <span className="font-semibold text-white">{rec.date}</span>
      ...
      <div className="p-4.5 rounded-2xl bg-[#161b22] border border-[#30363d] space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            <Sparkles className="w-4 h-4 text-purple-400" />
            <div>
              <h3 className="text-sm font-bold text-white">Debt Freedom Acceleration Simulator</h3>
              <p className="text-[11px] text-[#8b949e]">Compare Debt Avalanche (Highest APR First) vs. Debt Snowball (Lowest Balance First)</p>
            </div>
          </div>

          <div className="flex items-center gap-2 text-xs">
            <span className="text-[#8b949e]">Extra Monthly Payoff:</span>
            <input
              type="number"
              step="50"
              min="0"
              ...
              className="w-24 bg-[#0d1117] border border-[#30363d] focus:border-indigo-500 rounded-lg px-2 py-1 text-white font-mono text-xs focus:outline-none"
            />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
          {/* Avalanche Strategy */}
          <div className="p-3.5 rounded-xl bg-gradient-to-br from-indigo-950/40 to-purple-950/40 border border-indigo-500/30">
            <div className="flex items-center justify-between mb-1.5">
              <h4 className="font-bold text-indigo-300">🏔️ Debt Avalanche (Mathematically Optimal)</h4>
            ...
            <div className="grid grid-cols-2 gap-2 text-center font-mono">
              <div className="p-2 rounded-lg bg-[#0d1117]/80">
                <span className="text-[10px] text-[#8b949e] font-sans block">Debt-Free In</span>
                <span className="text-sm font-bold text-white">{debtSimulation.avalanche.months} Months</span>
              </div>
              <div className="p-2 rounded-lg bg-[#0d1117]/80">
                <span className="text-[10px] text-[#8b949e] font-sans block">Estimated Interest</span>
          ...
          {/* Snowball Strategy */}
          <div className="p-3.5 rounded-xl bg-gradient-to-br from-purple-950/40 to-pink-950/40 border border-purple-500/30">
            <div className="flex items-center justify-between mb-1.5">
              <h4 className="font-bold text-purple-300">⚡ Debt Snowball (Psychological Momentum)</h4>
            ...
            <div className="grid grid-cols-2 gap-2 text-center font-mono">
              <div className="p-2 rounded-lg bg-[#0d1117]/80">
                <span className="text-[10px] text-[#8b949e] font-sans block">Debt-Free In</span>
                <span className="text-sm font-bold text-white">{debtSimulation.snowball.months} Months</span>
              </div>
              <div className="p-2 rounded-lg bg-[#0d1117]/80">
                <span className="text-[10px] text-[#8b949e] font-sans block">Estimated Interest</span>
        ...
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-[#161b22] border border-[#30363d] text-white rounded-2xl w-full max-w-md p-4 space-y-4 shadow-2xl animate-fade-in text-xs">
            ...
            <form onSubmit={handleRecordPayment} className="space-y-3">
              ...
              <div>
                <label className="block text-[11px] font-semibold text-[#8b949e] mb-1">Payment Date</label>
                <input
                  type="date"
                  required
                  value={paymentDate}
              ...
              <div>
                <label className="block text-[11px] font-semibold text-[#8b949e] mb-1">Notes</label>
                <input
                  type="text"
                  value={paymentNotes}
