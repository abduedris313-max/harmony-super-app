...
export const LoanModal: React.FC<LoanModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-md flex items-center justify-center p-4">
      <div className="bg-[#161b22] border border-[#30363d] text-white rounded-2xl w-full max-w-lg max-h-[90vh] flex flex-col overflow-hidden shadow-2xl animate-fade-in">
        {/* Header */}
        <div className="p-4 border-b border-[#30363d] flex items-center justify-between bg-[#0d1117]">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[11px] text-[#8b949e]">EMI, Amortization & Repayment</p>
            </div>
          </div>
          <button
            onClick={onClose}
            className="p-1 rounded-full text-[#8b949e] hover:text-white hover:bg-[#21262d] transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-4 overflow-y-auto space-y-4 flex-1 scrollbar-none text-xs">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block text-[11px] font-semibold text-[#8b949e] mb-1">Loan Title *</label>
              <input
                type="text"
                required
                value={title}
            ...
            <div>
              <label className="block text-[11px] font-semibold text-[#8b949e] mb-1">Loan Type</label>
              <select
                value={type}
          ...
          {/* Lender / Institution */}
          <div>
            <label className="block text-[11px] font-semibold text-[#8b949e] mb-1">Lender / Financial Institution</label>
            <input
              type="text"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block text-[11px] font-semibold text-[#8b949e] mb-1">Original Principal ({currencyCode}) *</label>
              <input
                type="number"
                step="0.01"
                required
            ...
            <div>
              <label className="block text-[11px] font-semibold text-[#8b949e] mb-1">Current Remaining Balance ({currencyCode})</label>
              <input
                type="number"
                step="0.01"
                min="0"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block text-[11px] font-semibold text-[#8b949e] mb-1">Annual Interest Rate (APR %)</label>
              <div className="relative">
                <input
                  ...
                  placeholder="5.5"
                  className="w-full bg-[#0d1117] border border-[#30363d] focus:border-indigo-500 rounded-xl pl-3 pr-8 py-2 text-white font-mono focus:outline-none"
                />
                <span className="absolute right-3 top-2 text-[#8b949e] font-mono">%</span>
              </div>
            </div>

            <div>
              <label className="block text-[11px] font-semibold text-[#8b949e] mb-1">Tenure (Total Months)</label>
              <input
                type="number"
                min="1"
                max="600"
          ...
          <div className="p-3 rounded-xl bg-gradient-to-r from-indigo-950/60 to-purple-950/60 border border-indigo-500/30 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Calculator className="w-5 h-5 text-indigo-400" />
              <div>
                <span className="text-[10px] text-[#8b949e] uppercase font-bold tracking-wider">Calculated Monthly EMI</span>
                <p className="text-base font-bold text-white font-mono">
                  {formatCurrency(estimatedEmi, currencyCode)} <span className="text-xs font-normal text-indigo-300">/mo</span>
                </p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block text-[11px] font-semibold text-[#8b949e] mb-1">Loan Start Date</label>
              <input
                type="date"
                required
                value={startDate}
                onChange={(e) => setStartDate(e.target.value)}
                className="w-full bg-[#0d1117] border border-[#30363d] focus:border-indigo-500 rounded-xl px-3 py-2 text-white focus:outline-none font-mono"
              />
            </div>

            <div>
              <label className="block text-[11px] font-semibold text-[#8b949e] mb-1">Next Payment Due Date</label>
              <input
                type="date"
                required
                value={nextDueDate}
          ...
          {/* Notes */}
          <div>
            <label className="block text-[11px] font-semibold text-[#8b949e] mb-1">Notes / Terms</label>
            <textarea
              rows={2}
              value={notes}
              onChange={(e) => setNotes(e.target.value)}
              placeholder="Account reference numbers, fixed vs variable terms, early prepayment conditions..."
              className="w-full bg-[#0d1117] border border-[#30363d] focus:border-indigo-500 rounded-xl p-2.5 text-white focus:outline-none"
            />
          </div>
          ...
          <div className="pt-2 flex items-center justify-end gap-2 border-t border-[#30363d]">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2 rounded-xl bg-[#21262d] hover:bg-[#30363d] text-[#c9d1d9] font-medium transition-colors"
            >
              Cancel
            </button>
            <button
              ...
              className="px-5 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-semibold flex items-center gap-1.5 transition-colors disabled:opacity-50"
            >
